...
export function BrokerAccounts() {
  ...
  // FIX: Removed password from state. Use ref instead so it doesn't trigger
  // re-renders and is less visible in React DevTools.
  ...
  const connect = async () => {
    // FIX: Read password from ref instead of state
    ...
    // FIX: Clear form and password ref after successful connection
  ...
  const sync = async (account: BrokerAccount) => {
    ...
    try {
      await fetch(`/api/broker-accounts/${account.id}/sync`, { method: "POST" })
      await new Promise((resolve) => setTimeout(resolve, 1200))
      setAccounts((current) => current.map((item) => item.id === account.id ? { ...item, status: "Connected", lastSynced: "Just now" } : item))
    } catch {
      setAccounts((current) => current.map((item) => item.id === account.id ? { ...item, status: "Error", error: "We couldn't sync this account. Check the connection and try again." } : item))
    }
  }
  ...
  return <div className="space-y-6">
    <div className="flex items-start justify-between gap-4">
      ...
      <Button className="min-h-11 shrink-0 gap-2" onClick={() => setShowForm(true)}>
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-md">
        ...
        {progress !== "idle" ? <div className="flex flex-col items-center gap-4 py-10 text-center"><Loader2 className="h-9 w-9 animate-spin text-primary" /><div><p className="font-semibold">{progressLabel}</p><p className="mt-1 text-sm text-muted-foreground">This can take 30–90 seconds.</p></div><div className="flex gap-1">{["connecting", "verifying", "syncing"].map((step) => <span key={step} className={`h-1.5 w-14 rounded-full ${["connecting", "verifying", "syncing"].indexOf(step) <= ["connecting", "verifying", "syncing"].indexOf(progress) ? "bg-primary" : "bg-muted"}`} />)}</div></div> : <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {/* FIX: Use ref instead of state for password */}
            <Input
              className="text-base"
              id="password"
              type="password"
